...
def create_constraint_matrix():
    A = [[] for b in B]
    for b in B:
        brew_ingredients = brews_df.loc[b][['first_ingredient',
                                            'second_ingredient',
                                            'third_ingredient']]
...
def create_model(inventory, stats):
    ...
    for i in I:
        m += xsum(x[pair[2]] for pair in brew_ingredient_pairs 
                    if pair[1]==i) <= inventory[i]
    ...
    optimal_df = optimal_df[['name', 'count', 'value', 'first_ingredient', 
                             'second_ingredient', 'third_ingredient', 
                             'descriptions']]
